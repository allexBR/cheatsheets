...
install_common_dep() {
	...
	if ! run_command apt install -y --no-install-recommends --assume-yes \
		build-essential cmake curl gnupg pkg-config python3 python3-pip sudo; then
...
install_openvasd_dep() {
	...
	if ! run_command apt install -y \
		capnproto libcapnp-dev libmagic-dev libssl-dev pkg-config; then
...
build_install_opsd() {
	...
	if ! run_command python3 -m pip install --root="$INSTALL_DIR/$comp_name" --ignore-installed --no-warn-script-location .; then
		log ERROR "Failed to install $comp_name-$comp_ver via pip."
		exit 1
	fi
	mkdir -p "$INSTALL_DIR/$comp_name/tmp" && touch "$INSTALL_DIR/$comp_name/tmp/placeholder"
	if ! run_command cp -rv "$INSTALL_DIR/$comp_name"/* /; then
...
build_install_py() {
	...
	if ! run_command python3 -m pip install --root="$INSTALL_DIR/$comp_name" --ignore-installed --no-warn-script-location "$comp_name"; then
